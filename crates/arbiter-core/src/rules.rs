...
pub fn parse(text: &str) -> Result<Rule, ParseError> {
...
pub fn load_from_dir(dir: &Path) -> Vec<Rule> {
...
pub fn load_all(cwd: &str, extra_dirs: &[String]) -> Vec<Rule> {
    let mut all = Vec::new();
...
pub fn resolve<'a>(rules: &'a [Rule], scenario: Scenario, file: Option<&str>) -> Vec<&'a Rule> {
    rules
        .iter()
        .filter(|r| {
...
pub fn format_for_prompt(rules: &[&Rule]) -> String {
    if rules.is_empty() {
        return String::new();
    }
