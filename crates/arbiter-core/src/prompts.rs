...
#[derive(Debug)]
pub struct SimilarThreadContext {
    pub file: String,
    pub line: u32,
    pub status: String,
...
pub fn parse_extraction_response(response: &str) -> Vec<ExtractionAction> {
...
pub fn format_apply_feedback_prompt(
...
pub fn format_similarity_prompt(items: &[(usize, &str, u32, &str)]) -> Option<String> {
    if items.len() < 2 {
        return None;
    }
    let mut prompt = String::from(
...
pub fn parse_similarity_response(text: &str) -> Vec<Vec<usize>> {
    if text.trim() == "NONE" {
        return Vec::new();
    }
    text.lines()
